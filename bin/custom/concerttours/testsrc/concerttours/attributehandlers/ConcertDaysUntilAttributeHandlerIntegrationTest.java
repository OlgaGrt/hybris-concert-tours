...
public class ConcertDaysUntilAttributeHandlerIntegrationTest extends ServicelayerTransactionalTest
{
    ...
    public void testGetFutureConcertDate() throws Exception
    {
        ...
        Assert.assertEquals("Wrong value for concert in the future: " + concert.getDayUntil().longValue(), 2L, concert.getDayUntil().longValue());
    }
    @Test
    public void testGetNullConcertDate()
    {
        final ConcertModel concert = modelService.create(ConcertModel.class);
        Assert.assertNull("No concert date does not return null: "+ concert.getDayUntil(), concert.getDayUntil());
    }
    @Test
    public void testGetPastConcertDate() throws Exception
    {
        ...
        Assert.assertEquals("Wrong value for concert in the past: "+concert.getDayUntil().longValue(), 0L, concert.getDayUntil().longValue());
    }
}